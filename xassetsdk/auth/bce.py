...
def normalize_string(in_str, encoding_slash=True):
    ...
    if encoding_slash:
        encode_f = lambda c: NORMALIZED_CHAR_LIST[int(c)]
    else:
        # 仅仅在生成规范URI时。不需要对斜杠'/'进行编码
        encode_f = lambda c: NORMALIZED_CHAR_LIST[ord(chr(c))] if chr(c) != '/' else chr(c)
...
def sign(credentials, http_method, path, headers, params,
    ...
    sign_key = hmac.new(
        str.encode(credentials.secret_access_key),
        str(sign_key_info).encode('utf-8'),
    ...
    sign_result = hmac.new(str.encode(sign_key), str(string_to_sign).encode('utf-8'), hashlib.sha256).hexdigest()
